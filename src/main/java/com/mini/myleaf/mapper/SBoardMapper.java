...
public interface SBoardMapper {
	...
	//답글 등록
	@Insert("INSERT INTO sboard(sno, sname, stitle, scontent,suphit, sdate, groupno, depth, grouporder) VALUES(sno_seq.nextval,#{sname},#{stitle},#{scontent},0,sysdate,#{groupno}, #{depth}, #{grouporder})")
	public void sboardAnswer(SBoardVO sboardVO);
	
	//원글 등록
	@Insert("insert into sboard(sno,sname,stitle,scontent,suphit,sdate, groupno, depth, grouporder) values(sno_seq.nextval,#{sname},#{stitle},#{scontent},0,sysdate,sno_seq.currval, 1, 0)")
	public void sboardInsertOri(SBoardVO sboardVO);
